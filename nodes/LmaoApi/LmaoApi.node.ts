...
export class LmaoApi implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				displayOptions: {
					show: { mode: ['video'], videoOperation: ['create'], storyboardMode: [true] },
				},
				placeholder: '添加分镜',
				default: {
					shots: [
			...
			},
			{
				...
				type: 'string',
				displayOptions: {
					show: { mode: ['video'], videoOperation: ['remix', 'retrieve', 'download'] },
				},
				default: '',
				required: true,
			},
			...
			// --- 分辨率：图像生成 ---
			{
				...
				displayOptions: {
					show: {
						mode: ['image'],
						imageModel: ['gemini-3-pro-image-preview', 'gemini-3.1-flash-image-preview'],
					},
				},
				options: [
					{ name: '1K', value: '1K' },
			...
			},
			{
				...
				type: 'options',
				displayOptions: {
					show: { mode: ['image'], imageModel: ['gemini-3.1-flash-image-preview'] },
				},
				options: [
					{ name: '1:1', value: '1:1' },
					{ name: '1:4', value: '1:4' },
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				const binaryPropInput = this.getNodeParameter(
					'binaryPropertyName',
					i,
					'data, data0, data1, data2, file, attachment',
				) as string;
				const propNames = binaryPropInput
					.split(',')
					.map((s) => s.trim())
					.filter((s) => s !== '');
				...
				if (mode === 'text') {
					...
					// 获取 Binary 来源模式参数
					const binarySourceMode = this.getNodeParameter('binarySourceMode', i, 'current') as
						| 'current'
						| 'specified'
						| 'url';
					const sourceNodeNamesInput = this.getNodeParameter('sourceNodeNames', i, '') as string;
					const specifiedNodes = sourceNodeNamesInput
						.split(',')
						.map((s) => s.trim())
						.filter((s) => s !== '');
					...
					if (binarySourceMode === 'url') {
						...
						// 收集 Binary 数据
						const { binary: collectedBinary, bufferMap } = await collectBinaryFromNodes(
							this,
							i,
							binarySourceMode,
							specifiedNodes,
						);
						extractedImages = await extractImagesFromBinary(
							this,
							i,
							collectedBinary,
							propNames,
					...
					if (
						...
						const b64 = res.candidates?.[0]?.content?.parts?.find(
							(p: Record<string, unknown>) => p.inlineData,
						)?.inlineData.data;
						if (b64) {
							const outputFileName =
								imageModel === 'gemini-3.1-flash-image-preview'
									? 'gemini_flash_image.png'
									: 'gemini_image.png';
						...
						const images: string[] = extractedImages.map(
					...
					if (operation === 'create') {
						...
						if (storyboardMode) {
							const shotCollection = this.getNodeParameter('storyboardShots', i) as {
								shots?: Array<{ shotPrompt: string; duration: number }>;
							};
							if (shotCollection?.shots) {
								finalPrompt = shotCollection.shots
									.map((s, index) => {
					...
					const specifiedNodes = sourceNodeNamesInput
						...
						.map((s) => s.trim())
						.filter((s) => s !== '');
					const audioPropInput = this.getNodeParameter(
						'audioPropertyName',
						i,
						'data, data0, video, video0, audio, audio0',
					) as string;
					const propNames = audioPropInput
						.split(',')
						.map((s) => s.trim())
						.filter((s) => s !== '');
					...
					// 提取音频文件
					const audioData = await extractAudioFromBinary(
						this,
						i,
						collectedBinary,
						propNames,
